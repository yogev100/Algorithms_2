...
import Lec1._01_Bottles_Problem;
...
//		Print(T);
	...
	private static void PrintAllPath(boolean[][] T, String[][] path, int n) {
		for (int i = 0; i < path.length; i++) {
			for (int j = 0; j < path.length; j++) {
				System.out.print("("+_01_Bottles_Problem.getI(i, n)+","+_01_Bottles_Problem.getJ(i, n)+")"
				+ "->("+_01_Bottles_Problem.getI(j, n)+","+_01_Bottles_Problem.getJ(j, n)+"):\t");
				if (T[i][j] == true)
